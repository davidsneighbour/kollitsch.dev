...
* **Evidence**: verified against a local production build
  ...
  1. `getComputedStyle(input, '::placeholder').color` resolves to the
     authored colour (`--color-gray-600`, approximately `rgb(75, 88, 91)`).
  ...
  3. A real screenshot of the rendered input on `/find/` shows legible,
     dark placeholder text on a white input background.
  4. Manual WCAG luminance/contrast calculation on the rasterised colour
     ...
     theme) gives approximately 7.4:1, well past the 4.5:1 minimum.
* **Handling**: [#1835](https://github.com/davidsneighbour/kollitsch.dev/issues/1835)
  is the issue that added the real CSS fix. See
  `src/components/features/search/Search.astro`'s
  `.pagefind-ui__form .pagefind-ui__search-input::placeholder` rule for it
  (raised specificity via the `.pagefind-ui__form` ancestor, plus
  `!important`, to reliably beat Pagefind's own injected style).
  [#2032](https://github.com/davidsneighbour/kollitsch.dev/issues/2032)
  is the blocked follow-up tracking when this exclusion can be removed:
  periodically bump `@axe-core/playwright` and re-run
  `accessibility.spec.ts` without the exclusion. If a newer axe-core
  version passes cleanly, remove the exclusion, delete this chapter, and
  close that issue. Until then, this stays excluded rather than failing
  the live smoke test on a proven false positive.
