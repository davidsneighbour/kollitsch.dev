...
// ---------- Cache ----------
let cache: {
  files: ReadonlyMap<string, ImageRecord>;
...
async function loadIndex(): Promise<{
  ...
  try {
    ...
    const raw = (await import('/src/content/_generated/image-index.json'))
      .default as unknown;
...
export async function findImagesByTag(
  tag: string,
...
export async function findImagesInDir(
  dir: string,
  recursive = true,
): Promise<readonly ImageRecord[]> {
  ...
  return list.filter((r) => {
    ...
    return recursive
      ? r.dir === norm || r.dir.startsWith(`${norm}/`)
      : r.dir === norm;
  });
}

export function getPlaceholder(rec: ImageRecord): {
