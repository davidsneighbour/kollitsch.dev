...
async function getData(screenshotDir = ".github/screenshots", retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const screenshotPathDefault = path.join(__dirname, screenshotDir, `screenshot.png`);
      ...
      const screenshotPathWithTimestamp = path.join(__dirname, screenshotDir, `screenshot_${timestamp}.png`);
