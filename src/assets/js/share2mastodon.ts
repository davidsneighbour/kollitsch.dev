...
const DEFAULT_I18N: MastodonShareI18n = {
  ...
  errorInvalidInstance:
    'Please enter a valid instance URL (https://example.tld).',
  errorMissingConfig:
  ...
  instancePlaceholder: 'e.g. https://mastodon.social',
...
function parseI18n(el: HTMLElement): MastodonShareI18n {
  ...
  for (const key of Object.keys(DEFAULT_I18N) as (keyof MastodonShareI18n)[]) {
...
function readConfig(el: HTMLElement): {
  ...
  const urlTemplate = getAttr(el, 'data-url-template') ?? DEFAULT_URL_TEMPLATE;
...
class MastodonShare extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    if (stored !== null && this.#input) this.#input.value = `https://${stored}`;
  }
