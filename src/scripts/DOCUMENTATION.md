...
| `promote-stream-post.ts` | Converts a `/stream/` Mastodon post into a blog draft under `src/content/blog/YYYY/slug/index.md`, preserving Mastodon provenance in `origin` frontmatter (see `src/utils/mastodon-stream.ts` and `documentation/content/frontmatter.md`). Pass a Mastodon status id to promote it directly, or omit it to pick from a numbered list of recent eligible posts. Downloads media attachments into the post bundle by default (`--keep-remote-media` keeps remote links instead). `--offline` uses only `.cache/mastodon-stream/*.json`, never the network. Also supports `--dry-run`, `--force` (promote an ineligible status, or overwrite an existing file), `--no-open`, and `--title`/`--description`/`--tags`/`--slug` to skip the matching prompt. Loads `${HOME}/.env` then a local `.env` (local overrides) for `MASTODON_INSTANCE_URL`/`MASTODON_ACCOUNT_ID`/`MASTODON_ACCOUNT_HANDLE`/`MASTODON_ACCESS_TOKEN`. | `npm run promote:stream -- [id]` |
