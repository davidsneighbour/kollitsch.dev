...
function parseArgs(argv: readonly string[]): CliOptions {
	...
	const frontmatterDbPath = get('--frontmatter-db', '.frontmatter/database/mediaDb.json');
	...
	return { imagesDir, metaJsonPath, frontmatterDbPath, outPath, lqipWidth, concurrency };
}
...
async function statImage(fileAbs: string): Promise<{ width: number; height: number }> {
	const meta = await sharp(fileAbs).metadata();
	return { width: meta.width ?? 0, height: meta.height ?? 0 };
}

// ---------- Metadata readers ----------
async function readMetaMap(metaPath: string | null): Promise<Record<string, ImageMeta>> {
	if (!metaPath) return {};
	try {
...
async function readFrontmatterDbMap(pathStr: string | null): Promise<Record<string, ImageMeta>> {
	if (!pathStr) return {};
	try {
		...
		for (const [k, v] of Object.entries(imgMap)) {
			...
			out[k] = {
				...
				author: typeof (v as any).author === 'string' ? (v as any).author : undefined,
...
async function mergeMeta(metaPath: string | null, dbPath: string | null): Promise<Record<string, ImageMeta>> {
	const [flat, nested] = await Promise.all([readMetaMap(metaPath), readFrontmatterDbMap(dbPath)]);
	return { ...flat, ...nested };
}
...
async function buildIndex(opts: CliOptions): Promise<GeneratedIndex> {
	...
	const files = await glob('**/*.{jpg,jpeg,png,webp,avif,svg,gif}', { cwd: root, absolute: true });
	...
	const worker = async (): Promise<void> => {
		while (queue.length) {
			...
			const { width, height } = ext === 'svg' ? { width: 0, height: 0 } : await statImage(file);
			const lqipDataUri =
				ext === 'svg'
...
async function main(): Promise<void> {
	...
	await fs.writeFile(resolve(opts.outPath), JSON.stringify(index, null, 2), 'utf8');
	console.log(`Indexed ${Object.keys(index.files).length} images → ${opts.outPath}`);
}
